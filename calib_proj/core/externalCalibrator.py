...
class ExternalCalibrator: 
    ...
    def bootstrapping_from_initial_pair(self, 
        ...
        if 1:
            ...
            if decompositions[0]:
                ...
                if numbers_in_front.count(max_number_in_front) == 1: 
                    best_sol_idx = numbers_in_front.index(max_number_in_front)
                    best_cam = cam2_list[best_sol_idx]
                    best_plane = plane_list[best_sol_idx]
                  
                else: 
                    sol_indices = [i for i, n in enumerate(numbers_in_front) if n == max_number_in_front]

                    print("There are two solutions from decomposing the inter-image homography that have the same number of points triangulated with positive depth in all cameras.") 

                    
                    # select third camera 
                    candidates = self.get_remaining_camera_ids() - {cam_id_1, cam_id_2}
                    best_score = -np.inf
                    third_cam_id = None
                    for cam_id in candidates:
                        common_points_id = set(self.get_conform_obs_in_cam(cam_id).keys()) & self.estimate.scene.generic_scene.get_point_ids()
                    ...
                    if third_cam_id is None:
                        ...
                        return False
                    pts_cam3 = [self.correspondences[third_cam_id][point_id]._2d for point_id in self.get_conform_obs_in_cam(third_cam_id).keys()]
                    ...
                    for sol_idx in sol_indices:
                        cam2 = cam2_list[sol_idx]
                        n_1 = n1_list[sol_idx]


                        # triangulate points 
                        cam1_cam2_common_points_ids = list(set(self.get_conform_obs_in_cam(cam_id_1).keys()) & \
                            set(self.get_conform_obs_in_cam(cam_id_2).keys()))
                        if cam1_cam2_common_points_ids:
                            pts_src = [self.correspondences[cam_id_1][point_id]._2d for point_id in cam1_cam2_common_points_ids]
                            pts_dst = [self.correspondences[cam_id_2][point_id]._2d for point_id in cam1_cam2_common_points_ids]
                            pts_src = np.array(pts_src, dtype='float32').squeeze()
                            pts_dst = np.array(pts_dst, dtype='float32').squeeze()
                            points = cv2.triangulatePoints(cam1.get_projection_matrix(), 
                                                        cam2.get_projection_matrix(), 
                                                        pts_src.T, 
                                                        pts_dst.T)
                            points = points / points[3,:]
                            points = points[:3, :]

                        points_dict = {point_id: points[:,i] for i, point_id in enumerate(cam1_cam2_common_points_ids)}

                        # estimate third camera pose with PnP with triangulated points
                        K = self.intrinsics[third_cam_id].K
                        common_points_with_third_camera_id = set(self.get_conform_obs_in_cam(third_cam_id).keys()) & set(cam1_cam2_common_points_ids)
                        _2d = [self.correspondences[third_cam_id][point_id]._2d for point_id in list(common_points_with_third_camera_id)]
                        _3d = [points_dict[point_id] for point_id in list(common_points_with_third_camera_id)]
                        _2d = np.vstack(_2d)
                        _3d = np.vstack(_3d)
                        T_W_C3 = self.pnp(_2d, _3d, K)
                        print("T_W_C3:")
                        print(T_W_C3)
                        T_C3_W = se3.inv_T(T_W_C3)
                        R_3_1 = T_C3_W[:3,:3]

                        # count the points in front of third camera
                        in_front_cam3 = []
                        n_3 = R_3_1 @ n_1
                        print(f"n_3: {n_3}")
                        for pt in pts_cam3:
                            m = np.linalg.inv(self.intrinsics[third_cam_id].K) @ np.append(pt,1)
                            in_front_cam3.append(m.dot(n_3) > 0)
                        number_in_front_cam3 = int(sum(in_front_cam3))
                        point_in_front_of_third_camera[sol_idx] = number_in_front_cam3

                        print(f"Solution {sol_idx+1} : number of points in front of third camera {third_cam_id}: {number_in_front_cam3}")

                    best_sol_idx = max(point_in_front_of_third_camera, key=point_in_front_of_third_camera.get)
                    best_cam = cam2_list[best_sol_idx]
                    best_plane = plane_list[best_sol_idx]
    ...
    def estimate_inter_image_homography_normalized(self, 
                                                    cam_id_1: idtype, 
                                                    cam_id_2: idtype): 
        ...
        if len(common_points_ids) >= 4: 
            ...
            pts_dst_normalized = (np.linalg.inv(K_2) @ np.vstack((pts_dst.T, np.ones(pts_dst.shape[0]))) )[:2].T
            ...
            print(f"Less than 4 common points btw cam {cam_id_1} and {cam_id_2}.")
            if self.config.SOLVING_LEVEL == SolvingLevel.FREE or self.config.SOLVING_LEVEL == SolvingLevel.PLANARITY:
